...
# --- LÓGICA DE TRANSPOSICIÓN Y PROCESAMIENTO ---
...
def procesar_texto_final(texto, semitonos):
    ...
    for linea in texto.split('\n'):
        if not linea.strip(): 
            ...
            continue
        # Detección de línea de acordes por densidad de espacios
...
if menu == "🏠 Cantar / Vivo":
    ...
    if not df_v.empty:
        ...
        # VISTA CON TÍTULO, AUTOR Y CATEGORÍA
        st.markdown(f'''<div class="visor-musical">
            <h2 style="margin:0; color:inherit;">{data["Título"]}</h2>
            <p style="margin:0; opacity:0.8;">{data["Autor"]} | <b>{data["Categoría"]}</b></p>
            <hr>{html_cuerpo}</div>''', unsafe_allow_html=True)
    ...
    if l_n:
        ...
        previa_html = procesar_texto_final(l_n, 0)
        st.markdown(f'<div class="visor-musical">{previa_html}</div>', unsafe_allow_html=True)
    ...
    if st.button("💾 Guardar Canción"):
        ...
        else: st.error("Título y Letra son obligatorios.")
    ...
    for c in categorias:
        ...
        col_c1.write(f"• {c}")
        if col_c2.button("Eliminar", key=f"del_{c}"):
            categorias.remove(c)
            guardar_categorias(categorias); st.rerun()
            
    st.markdown("---")
    n_cat = st.text_input("Añadir nueva categoría:")
    if st.button("Añadir"):
